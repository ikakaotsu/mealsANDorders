...
const renderOrder = (order, meals) => {
  ...
  const element = stringToHTML(
    `<li data-id="${order._id}"><span>${meal.name} - ${order.user_id}</span></li>`
  );

  element.addEventListener("click", () => {
    const ordersList = document.getElementById("orders-list");
    Array.from(ordersList.children).forEach((x) =>
      x.classList.remove("chosen-one")
    );
...
const deleteOreders = () => {
  ...
  delform.onsubmit = (e) => {
    ...
    fetch("http://localhost:3000/api/orders/=id=" + orderIdValue, {
      method: "DELETE",
      headers: {
